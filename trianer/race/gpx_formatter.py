...
class Point:
    ...
    def delete_tag(self, tag):
        if not self.is_valid_gpx or tag not in self.npoint:
            return
        point = self.npoint
        self.npoint = point[: point.find(tag)] + point[point.find(tag.replace("<", "</")) + len(tag) + 1 :]
...
class GpxFormatter:
    ...
    def apply_filter(self, point, filter):
        ...
        if ctime >= filter["min_time"] and ctime <= filter["max_time"]:
            ...
            print("%s: Altitude %s offset to %sm" % (self.filename, point.dtime.time(), int(altitude_offset)))
